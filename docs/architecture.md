...
| --- | --- | --- |
| Run bundle | `src/scenariolens/run_bundle.py` | Expands input directories, records public-safe input provenance, executes the core baseline/lane studies, and writes one versioned top-level manifest and report with a deterministic analysis digest. |
...
| Lane-continuation terminal neighborhood selector transfer | `src/scenariolens/lane_continuation_terminal_neighborhood_selector_transfer.py` | Applies a calibrated selector policy to a broader replay queue, separating calibration overlap from novel validation cases and reporting transfer false holds/promotions without changing defaults. |
| Lane-continuation terminal neighborhood selector error audit | `src/scenariolens/lane_continuation_terminal_neighborhood_selector_error_audit.py` | Explains selector transfer false holds/promotions and sweeps small counterfactual gates so remaining errors become an evidence queue rather than an untracked tuning note. |
